...
- `RESONATES_WITH` edges link arcs from _different_ shows that share the same emotional knot,
...
4. **Choose** (LLM call #1, may see spoilers) — pass the top ~15 arc nodes _including_ `spoiler`
...
- `start_at` is where the arc _begins_, not where to start watching. Say "her journey begins in
