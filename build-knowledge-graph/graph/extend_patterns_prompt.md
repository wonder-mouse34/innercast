...
story PATTERNS under broad THEMES. The hierarchy already covers 194 arcs from 7 shows; 13 shows
were just added.

Files (all in graph/):

- `arc_index_new.txt` — the NEW arcs to place (one per line:
...
- `build_patterns.py` — the source of truth: THEMES (13), PATTERNS (60) and ASSIGN
...
2. Only if a new arc truly fits nothing, add a new PATTERN `(slug, label, description, [theme slugs])`
   ...
   pattern to a fitting OLD arc to achieve this). Keep new patterns to a minimum (aim ≤ 10).
