...
Self-contained: needs only Python 3 and graph.json in this same folder.
...
def find_matching_characters(situations=(), conflicts=(), emotions=(), patterns=(), avoid_warnings=(),
                             max_intensity="heavy", formats=(), exclude_shows=(), top=6):
    ...
    for p in picked[:3]:
        for e in OUT[p["arc_id"]]:
            ...
            if e["type"] == "RESONATES_WITH" and NODES[t]["show"] not in shows and allowed(t):
                ...
                break
    return {"candidates": picked, "wildcards": wildcards, "unknown_terms": unknown}
